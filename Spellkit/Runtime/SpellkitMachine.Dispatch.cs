...
internal static partial class SpellkitMachine
{
    ...
    private static VmDispatchResult ExecuteInfrastructure(Op op, VmState state)
    {
        ...
        if (op.Code is OpCode.SuspendSelect)
        {
            ...
            var factory = target as SpellkitSelectFactory;
            if (factory is null && target is SpellkitString name)
            {
    ...
    private static VmDispatchResult ExecuteLoadStore(Op op, VmState state)
    {
        switch (op.Code)
        {
            ...
            case OpCode.LoadConst:
                ...
                break;
            case OpCode.LoadNil:
                state.EvalStack.Push(SpellkitNil.Instance);
                break;
            ...
            case OpCode.LoadInt0:
                state.EvalStack.Push(SpellkitInteger.Zero);
                break;
            case OpCode.LoadInt1:
                state.EvalStack.Push(SpellkitInteger.One);
                break;
            case OpCode.LoadFloat0:
                state.EvalStack.Push(SpellkitFloat.Zero);
                break;
            case OpCode.LoadFloat1:
                state.EvalStack.Push(SpellkitFloat.One);
                break;
            ...
            case OpCode.LoadExternal:
                ...
                if (state.Second is SpellkitFunction function && function.Auto)
                {
                    state.Second = function.TryInvokeProperty(state.Context, SpellkitNil.Instance);
                }
    ...
    private static VmDispatchResult ExecuteBinary(Op op, VmState state) =>
        SpellkitMachineHelpers.ExecuteBinaryOperation(
    ...
    {
        state.Second = null;
        return SpellkitMachineHelpers.ExecuteUnaryOperation(op.Code, state.EvalStack, state.Types, state.Context)
            ? VmDispatchResult.Continue
            : new(VmStep.HandleCallback);
    }
    ...
    private static VmDispatchResult ExecuteFunction(Op op, VmState state)
    {
        switch (op.Code)
        {
            case OpCode.CreateFunction:
                state.EvalStack.Push(SpellkitMachineHelpers.ExecuteFunctionCreation(
            ...
            case OpCode.SetFunctionAttribute:
                ((SpellkitFunction)state.EvalStack.Peek()).Attr |= op.Data;
                break;
            case OpCode.CreateSelectFactory:
                var definition = state.Unit.Objects[op.Data] as SpellkitSelectDefinitionValue
                    ?? throw new InvalidOperationException("The select definition is unavailable.");
                var closures = new SpellkitFunction[op.Data2];
                for (var i = closures.Length - 1; i >= 0; i--)
                {
                    closures[i] = state.EvalStack.Pop() as SpellkitFunction
                        ?? throw new InvalidOperationException("A select closure is unavailable.");
                }
                state.EvalStack.Push(new SpellkitSelectFactory(definition, closures));
                break;
            case OpCode.CreateSelectFactoryTemplate:
                var initializer = state.EvalStack.Pop() as SpellkitFunction
                    ?? throw new InvalidOperationException("A select factory initializer is unavailable.");
                var name = state.Unit.Objects[op.Data] as SpellkitString
                    ?? throw new InvalidOperationException("The select factory name is unavailable.");
                state.EvalStack.Push(new SpellkitSelectFactory(name.Value, initializer));
                break;
            default:
                throw UnexpectedOpcode(op);
        }
    ...
    private static VmDispatchResult ExecuteMember(Op op, VmState state)
    {
        switch (op.Code)
        {
            ...
            case OpCode.LoadMember:
                return SpellkitMachineHelpers.ExecuteNamedMemberAccess(
            ...
            case OpCode.StoreMember:
                return SpellkitMachineHelpers.ExecuteTypeMemberMutation(
    ...
    private static VmDispatchResult ExecuteIndex(Op op, VmState state)
    {
        ...
        {
            OpCode.LoadIndex => SpellkitMachineHelpers.ExecuteIndexedAccess(
                state.EvalStack, state.Types, state.Context, out state.First, out state.Second),
            OpCode.LoadRawIndex => SpellkitMachineHelpers.ExecuteRawIndexedAccess(
                state.EvalStack, state.Types, state.Context, out state.First, out state.Second),
            OpCode.StoreIndex => SpellkitMachineHelpers.ExecuteIndexedSet(
                state.EvalStack, state.Types, state.Context,
                out state.First, out state.Second, out state.Third),
            OpCode.StoreRawIndex => SpellkitMachineHelpers.ExecuteRawIndexedSet(
                state.EvalStack, state.Types, state.Context,
                out state.First, out state.Second, out state.Third),
    ...
    private static VmDispatchResult ExecuteModuleInstruction(Op op, VmState state)
    {
        switch (op.Code)
        {
            case OpCode.LoadModule:
                ...
                state.EvalStack.Push(new SpellkitModule(
            ...
            case OpCode.FinishModule:
                ...
                {
                    throw new SpellkitRuntimeException(MessageCatalog.Get(MessageGroup.Runtime, "StackCorrupted.0"));
                }
    ...
    private static VmDispatchResult ExecuteIterator(Op op, VmState state)
    {
        switch (op.Code)
        {
            case OpCode.CreateIterator:
                state.EvalStack.Push(SpellkitMachineHelpers.ExecuteIteratorCreation(
                    state.Function, op.Data, state.Locals));
                break;
            case OpCode.LoadTerminator:
                state.EvalStack.Push(SpellkitNil.Terminator);
                break;
            case OpCode.Yield:
            ...
            case OpCode.JumpIfTerminator:
                if (ReferenceEquals(state.EvalStack.Peek(), SpellkitNil.Terminator))
                {
                    state.Offset = op.Data;
                }
            ...
            case OpCode.JumpIfIteratorValue:
                if (state.EvalStack.Peek().TypeId == SpellkitTypeCodes.Iterator)
                {
                    state.Offset = op.Data;
                }
            ...
            case OpCode.LoadIteratorFunction:
                ...
                if (state.Second is SpellkitIterator iterator)
                {
                    state.EvalStack.Replace(iterator.GetIteratorFunction());
                }
                else
                {
                    state.Context.InvalidType(SpellkitTypeCodes.Iterator, state.Second);
                    return new(VmStep.Throw);
                }
                break;
            default:
    ...
    private static VmDispatchResult ExecuteCall(Op op, VmState state)
    {
        switch (op.Code)
        {
            case OpCode.PrepareCall:
                return SpellkitMachineHelpers.TryPrepareFunction(
                    state.EvalStack, state.Types, state.Context, op.Data, out _)
                    ? VmDispatchResult.Continue
                    : new(VmStep.Throw);
            case OpCode.SetCallArgument:
                SpellkitMachineHelpers.AssignIndexedArgument(
            ...
            case OpCode.SetNamedCallArgument:
                ...
                var function = (SpellkitFunction)state.EvalStack.Peek(2);
                return SpellkitMachineHelpers.TryAssignNamedArgument(
            ...
            case OpCode.TailCall:
                ...
                SpellkitMachineHelpers.PrepareStandardCall(
    ...
    private static VmDispatchResult ExecuteCollection(Op op, VmState state)
    {
        switch (op.Code)
        {
            case OpCode.CreateLabel:
                state.EvalStack.Replace(new SpellkitLabel(
                    (string)state.Unit.Strings[op.Data], state.EvalStack.Peek()));
                return VmDispatchResult.Continue;
            case OpCode.CreateArguments:
                state.EvalStack.Push(op.Data == 0
                    ? SpellkitTuple.Empty
                    : SpellkitMachineHelpers.MakeTuple(state.EvalStack, op.Data, true));
                return VmDispatchResult.Continue;
            case OpCode.CreateTuple:
                state.EvalStack.Push(op.Data == 0
                    ? SpellkitTuple.Empty
                    : SpellkitMachineHelpers.MakeTuple(state.EvalStack, op.Data, false));
                return VmDispatchResult.Continue;
            case OpCode.CreateDictionary:
                state.EvalStack.Push(SpellkitMachineHelpers.MakeDictionary(state.EvalStack, op.Data));
                return VmDispatchResult.Continue;
            case OpCode.CheckContains:
                state.Second = state.Unit.Objects[op.Data];
                return SpellkitMachineHelpers.ExecuteContains(
    ...
    private static VmDispatchResult ExecuteConversion(Op op, VmState state)
    {
        ...
        state.Second = null;
        return SpellkitMachineHelpers.ExecuteStringConversion(
            state.EvalStack, state.Types, state.Context)
            ? VmDispatchResult.Continue
            : new(VmStep.HandleCallback);
    }
    ...
    private static VmDispatchResult ExecuteMetadata(Op op, VmState state)
    {
        switch (op.Code)
        {
            case OpCode.AddTypeAnnotation:
                ...
                state.Second = state.EvalStack.Peek();
                ((SpellkitLabel)state.Second).AddTypeAnnotation((SpellkitTypeInfo)state.First);
                break;
            case OpCode.MarkMutable:
                ((SpellkitLabel)state.EvalStack.Peek()).Mutable = true;
                break;
            default:
                throw UnexpectedOpcode(op);
        }
    ...
    private static VmDispatchResult ExecuteCast(Op op, VmState state)
    {
        switch (op.Code)
        {
            case OpCode.CreateCast:
                SpellkitMachineHelpers.ExecuteCastRegistration(
                    state.EvalStack, out state.First, out state.Second);
                return VmDispatchResult.Continue;
            case OpCode.ApplyCast:
                return SpellkitMachineHelpers.ExecuteCast(
                    state.EvalStack, state.Types, state.Context,
                    out state.First, out state.Second)
                    ? VmDispatchResult.Continue
                    : new(VmStep.Throw);
